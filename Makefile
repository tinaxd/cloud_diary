# This Makefile is for the cloud_diary extension to perl.
#
...
#     CONFIGURE_REQUIRES => {  }
#     NAME => q[cloud_diary]
#     PREREQ_PM => { DBD::SQLite=>q[1.74], DBIx::Connector=>q[0.59], Mojolicious=>q[9.38] }
#     TEST_REQUIRES => {  }
#     VERSION => q[0.01]
#     test => { TESTS=>q[t/*.t] }
...
MAN3SECTION = 3
INSTALLDIRS = site
INSTALL_BASE = /home/tinaxd/perl5
DESTDIR = 
PREFIX = $(INSTALL_BASE)
...
TO_INST_PM = lib/CloudDiary.pm \
	lib/CloudDiary/Controller/Diary.pm \
	lib/CloudDiary/Controller/Example.pm \
	lib/CloudDiary/DB/Conn.pm
...
DISTNAME = cloud_diary
DISTVNAME = cloud_diary-0.01
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'name: cloud_diary' >> META_new.yml
	$(NOECHO) $(ECHO) 'no_index:' >> META_new.yml
	$(NOECHO) $(ECHO) '  directory:' >> META_new.yml
	$(NOECHO) $(ECHO) '    - t' >> META_new.yml
	$(NOECHO) $(ECHO) '    - inc' >> META_new.yml
	$(NOECHO) $(ECHO) 'requires:' >> META_new.yml
	$(NOECHO) $(ECHO) '  DBD::SQLite: '\''1.74'\''' >> META_new.yml
	$(NOECHO) $(ECHO) '  DBIx::Connector: '\''0.59'\''' >> META_new.yml
	$(NOECHO) $(ECHO) '  Mojolicious: '\''9.38'\''' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "name" : "cloud_diary",' >> META_new.json
	...
	$(NOECHO) $(ECHO) '         "requires" : {' >> META_new.json
	$(NOECHO) $(ECHO) '            "DBD::SQLite" : "1.74",' >> META_new.json
	$(NOECHO) $(ECHO) '            "DBIx::Connector" : "0.59",' >> META_new.json
	$(NOECHO) $(ECHO) '            "Mojolicious" : "9.38"' >> META_new.json
	$(NOECHO) $(ECHO) '         }' >> META_new.json
	$(NOECHO) $(ECHO) '      }' >> META_new.json
	$(NOECHO) $(ECHO) '   },' >> META_new.json
...
ppd :
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="cloud_diary" VERSION="0.01">' > cloud_diary.ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT></ABSTRACT>' >> cloud_diary.ppd
	$(NOECHO) $(ECHO) '    <AUTHOR></AUTHOR>' >> cloud_diary.ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> cloud_diary.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="DBD::SQLite" VERSION="1.74" />' >> cloud_diary.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="DBIx::Connector" VERSION="0.59" />' >> cloud_diary.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Mojolicious::" VERSION="9.38" />' >> cloud_diary.ppd
	$(NOECHO) $(ECHO) '        <ARCHITECTURE NAME="x86_64-linux-gnu-thread-multi-5.34" />' >> cloud_diary.ppd
	$(NOECHO) $(ECHO) '        <CODEBASE HREF="" />' >> cloud_diary.ppd
	$(NOECHO) $(ECHO) '    </IMPLEMENTATION>' >> cloud_diary.ppd
	$(NOECHO) $(ECHO) '</SOFTPKG>' >> cloud_diary.ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  'lib/CloudDiary.pm' 'blib/lib/CloudDiary.pm' \
	  'lib/CloudDiary/Controller/Diary.pm' 'blib/lib/CloudDiary/Controller/Diary.pm' \
	  'lib/CloudDiary/Controller/Example.pm' 'blib/lib/CloudDiary/Controller/Example.pm' \
	  'lib/CloudDiary/DB/Conn.pm' 'blib/lib/CloudDiary/DB/Conn.pm' 
